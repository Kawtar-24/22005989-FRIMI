...
La fraude bancaire constitue un problème majeur dans les transactions digitales, causant des pertes financières considérables. Le défi principal réside dans le déséquilibre extrême des datasets : les transactions frauduleuses représentent moins de 0,2% du volume total. Ce déséquilibre biaise les modèles ML vers la classe majoritaire, compromettant la détection des fraudes. L'objectif est d'évaluer différentes techniques de rééquilibrage (undersampling, SMOTE) et modèles de classification pour optimiser la détection tout en gérant ce déséquilibre critique.
